...
class ViewPagerAdapter(var items: ArrayList<String> = arrayListOf()) : RecyclerView.Adapter<ViewPagerAdapter.ViewHolder>() {


    inner class ViewHolder(parent: ViewGroup) : RecyclerView.ViewHolder
        (LayoutInflater.from(parent.context).inflate(R.layout.viewpager_item, parent, false)) {
        val aespaMember = itemView.findViewById<TextView>(R.id.test_text)
    }

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int) = this.ViewHolder((parent))
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
       holder.aespaMember.setText("123")
    }
